...
live here, in the project**, grounded in *your* users and research.
...
1. **Grounded over invented.** Prefer personas built from real research (interviews,
   analytics, support tickets). If drafted from a brief without research, set
   `grounded: false` — the skill labels its findings *"not grounded in research"*.
2. **A goal-bearing user, not a demographic.** Lead with motivation, context, and mindset —
   what this person is *trying to do* and *why*, over age/income.
...
5. **Distinct from the others.** Each persona stresses a *different* axis (tech comfort,
   urgency, accessibility need, domain expertise, device). Aim for coverage, not a crowd.
6. **Tied to a scenario.** Every persona carries at least one concrete goal the skill turns
   into a walkthrough scenario ("buy one item as a guest, in a hurry").
...
reacts. Write it as if briefing an actor.
